...
| World topology | `TestWorldTopology.cpp` | Infinite/finite validation, canonical aliases, clipped finite presentation with blank out-of-bounds camera space, Life horizontal/vertical/corner seams, Wireworld seams, dual-grid topology matching, and a focused Release comparison micro-benchmark |
| Configuration menu | `TestConfigurationMenu.cpp` | Numeric and `inf` parsing, mixed-axis rejection, ruleset/navigation actions, deterministic reveal/selection/value animation state, readable high-contrast text composition, Release-sized primitive token emission, and hidden-state suppression |
...
| CellGameModule | `TestCellGameModule.cpp` | Editing/commands, Release F1 workflow, setting application, and Exit shutdown request, non-blocking one-in-flight scheduling and debt dropping, requested/achieved TPS and rolling timing status, async pause/save/load/ruleset/manual-step drains, full-source randomization at far zoom, centered seeds, sparse v3 topology round trip, v2 compatibility, legacy import, validation, camera restoration |
