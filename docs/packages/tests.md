...
| Services / UI / allocators | `TestServices.cpp`, `TestRuntimeUtilities.cpp`, `TestUITokens.cpp`, `TestAllocators.cpp`, `TestGameVisual.cpp` | Input, camera, console, presentation pacing defaults and FPS-label semantics, allocators, primitive composition |
...
performance assertion. The suite does not replace a manual Windows/OpenGL smoke
test.
