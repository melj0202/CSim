...
\pathref{Rulesets/} & Polymorphic CA rules; \pathref{AllSets.h} umbrella. \\
\pathref{Rendering/} & Window, camera, drawables, scene, RHI-ish interfaces, OpenGL backend. \\
\pathref{Services/} & Log, env vars, input, CLI, allocators, \symbolref{SaveLoad} API. \\
...
\pathref{Tests/} & Early unit-test scratch (not fully wired into CMake). \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \pathref{archive/} --- historical code (old State pattern, analyzer dumps). Not built.
\end{itemize}
