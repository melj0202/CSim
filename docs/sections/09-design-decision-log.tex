...
%----------------------------------------------------------------------------
\subsection*{D-P4 --- R8 palette + dirty-rect PBO uploads}
\begin{decisionbox}[title={D-P4 \quad 2026-07-28}]
\textbf{Decision:} Present cells as \texttt{GL\_R8} texture of state bytes plus
a $256\times 1$ RGB palette (\texttt{evalCell}). Upload dirty rects via PBO
ping-pong in \texttt{GLTexture::UpdateSubImage}. Drop dual float RGB staging.\\
\textbf{Why:} Large grids: bandwidth and CPU of RGB float fade dominated;
palette + R8 is $O(1)$ bytes per cell on CPU.\\
\textbf{Consequences:} Colors snap (no CPU fade); ruleset change rebuilds
palette only; shaders \pathref{canvas\_*.glsl}; bind-state tracker in
\symbolref{GLDevice}.
\end{decisionbox}
...
\symbolref{Scene}/\symbolref{IllumoContext}. \texttt{ObjectID} for the
lightweight scene graph lives on \pathref{SceneObject.h}.\\
