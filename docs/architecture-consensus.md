...
- Main risks: fixed command-queue overflow effects, native platform gaps, and
  explicit resource/failure handling at rendering and service boundaries
...
| **Primitives / GameVisual** | Value-type shapes/sprites/text on a `GameVisual` host (D-R15). **CanvasView**, **CommandLine**, **GLString**/SplashText, and **Cursor** embed/compose via `GameVisual`; dense Canvas is a compatibility fixture. |
...
| Tokens / Renderer | `Illumo/Source/Rendering/Renderer.h`, `RenderCommand.h` |
