...
\pagestyle{fancy}
\fancyhf{}
\lhead{\small\textcolor{slate}{\textbf{CSim} --- Architecture Consensus (Unified)}}
\rhead{\small\textcolor{slate}{2026-07-30}}
\cfoot{\thepage}
...
% --- Title Block ---
\begin{center}
    {\LARGE \bfseries \textcolor{navyblue}{CSim --- Architecture Consensus (Unified)}} \\[0.4em]
    {\small \textbf{Status:} Single living document --- \textbf{authoritative for later sessions} \quad|\quad \textbf{Last updated:} 2026-07-30}
\end{center}
...
\midrule
Desktop ``Main agenda for CSim'' & Product wishlist $\rightarrow$ \S7 scorecard \\
\texttt{CSim\_Architecture\_Decisions.pdf} (ChatGPT) & CA / OOP-practice design review $\rightarrow$ purpose, ownership, input, rules, SYCL \\
\texttt{CSim\_Engine\_Architecture\_Decisions.pdf} (ChatGPT) & Aspirational 2D engine $\rightarrow$ what we kept vs discarded \\
\texttt{gpt\_csim\_arch\_assessment.pdf} (ChatGPT) & Assessment of \textit{current} tree $\rightarrow$ risks, direction \\
...
\textbf{Current code under \texttt{CSim/Source/}} & \textbf{Wins} when anything conflicts \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    \item \texttt{docs/csim-design-notes.tex} / \texttt{.pdf} --- long-form chapters
...
\subsection{Desktop agenda (``Main agenda for CSim'')}
...
\subsection{CA design review (\texttt{CSim\_Architecture\_Decisions.pdf})}
...
\noindent \textbf{Principle:} \textit{Abstract real volatility; hard-code stable structure; ownership explicit.} \quad (Closer to current CSim than the ``engine passes'' PDF.)

\subsection{Engine design PDF (\texttt{CSim\_Engine\_Architecture\_Decisions.pdf})}
...
\begin{tabularx}{\textwidth}{l X}
\toprule
\textbf{Proposed} & \textbf{Status in CSim} \\
\midrule
...
\begin{itemize}
    ...
    \item Main risks: incomplete module Start handling, Canvas dual role, partial backend neutrality, CMake duplication, docs lagging code (especially Canvas presentation)
    \item Highest value: product correctness + one canonical Canvas model in docs --- \textbf{not} ECS or multi-pass
\end{itemize}

% --- Section 3 ---
\section{Overall assessment}
CSim is a \textbf{coherent layered modular monolith}:
...
\textbf{Tests} & Headless \texttt{CSimTests}: rules, canvas, tokens, UI, renderer inject. \\
...
\begin{enumerate}
    ...
    \item Modes in \texttt{CellGameModule}: \texttt{NORMAL | EDIT | SAVE | LOAD | EXIT} (enum + switch).
\end{enumerate}
...
2 & bug & \textbf{\texttt{LoadCellGame} skips dirty/fade.} Writes \texttt{lifeCanvas} directly; never \texttt{markCellsDirty()}. Display can stay stale until something else dirties the grid. \\
3 & bug & \textbf{\texttt{IllumoContextHasGameCore} omits \texttt{commandLine}}, but Edit always uses \texttt{ic->commandLine->isOpen}. \\
4 & bug & \textbf{Wireworld editor can't place heads.} Paint is conductor/empty only; no head/tail tool. \\
5 & bug & \textbf{Startup seed is always a GoL glider (value 0).} Under WIREWORLD that plants five electron heads, not a wire. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{\#} & \textbf{Issue} \\
\midrule
6 & Console ruleset list still duplicated vs \texttt{CellContext::IsKnownModeString} (drift risk). \\
7 & Wireworld tests miss 2-head birth / 3-head no-birth; no load$\rightarrow$dirty coverage. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    ...
    \item CMake source-list duplication (\texttt{CSim} vs \texttt{CSimTests}); empty package CMake files.
...
\begin{enumerate}
    \item Failed \texttt{Start} $\rightarrow$ do not run that module's Update/Dispatch; include \texttt{commandLine} in game-core check.
    \item Load path $\rightarrow$ \texttt{markCellsDirty} + visual rebuild.
    \item Wireworld seed + head placement UX.
    \item Keep this file and LaTeX ``current state'' sections aligned with \textbf{RGB fade Canvas}.
\end{enumerate}

\subsection{B. Hygiene}
\begin{enumerate}[resume]
    \item Single source of truth for known ruleset names.
    \item Shared CMake target for common sources.
