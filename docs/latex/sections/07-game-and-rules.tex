...
\symbolref{CommandRegistry}; Services remains free of Game types. Save writes a
bounded ruleset tag, dimensions, and dense cell bytes. Load validates the full
payload before mutation, activates the saved ruleset, copies the overlapping
region row-by-row, marks the Canvas dirty, rebuilds palette targets, and snaps
the display to the loaded state. Native picker variants route through the
platform \symbolref{SaveLoad} API. See D-CLI1 and
...
\subsection{Canvas (domain + view + GPU --- D-C2)}
\begin{itemize}
  \item CPU grid: \symbolref{CellGrid} owns \symbolref{lifeCanvas} (one byte
        per cell), dense rectangle (not chunked); \symbolref{Canvas} extends
        it for presentation.
  ...
  \item D-C2 refines D-C1's intentional monolith: \symbolref{Canvas} remains
        the presentation/GPU adapter, and a further view split waits for a real
        scale or product need.
\end{itemize}
...
pass for a dirty AABB during eval, then buffer-swap promote (D-P5; still $O(WH)$ scan).
