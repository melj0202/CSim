...
\begin{itemize}
  \item Domain: \symbolref{SparseCellGrid} stores signed-coordinate
        16$\times$16 chunks; the view samples only the camera-visible
        \texttt{CanvasX}$\times$\texttt{CanvasY} cells.
  \item View: CPU palette via \texttt{evalCell} $\rightarrow$ \texttt{targetRgb};
        ...
        and packed RGB is staged for upload.
  \item GPU: one reusable RGB display texture and one screen-space quad,
        using linear sampling when stretched to the window, and updated via
        at most one dirty-rect \texttt{UpdateTexture} token per submission
        (PBO path; optional \texttt{srcRowStride}).
