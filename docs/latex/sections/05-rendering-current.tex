...
\begin{itemize}
  ...
  \item \textbf{RenderStyle} --- Renderer-owned table of shader handles and
        \texttt{PipelineState} defaults. Current entries are \texttt{Canvas},
        \texttt{UiText}, and \texttt{Console}; GPU programs remain backend-owned.
  ...
  \item Production emitters use \texttt{AppendCommands} only; their
        \texttt{DrawImpl} fallback is empty. This covers CanvasView,
        CommandLine, GLString, and SplashText.
...
\texttt{CreateTexture} (handles = \texttt{unsigned long} table IDs).
...
Texture sub-image updates use PBO ping-pong and dirty-rect packing, with a
client-memory \texttt{glTexSubImage2D} fallback when mapping fails.
