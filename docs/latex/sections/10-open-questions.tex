...
\begin{enumerate}
  ...
  \item \sout{\textbf{Resource ownership:} refcounts, generational handles, or
        ...
        \textbf{Resolved D-R16/D-R17:} generational backend resources plus
        reference-counted managed texture/shader assets.
  ...
  \item \textbf{Linux/macOS:} invest now or freeze until token GL path is solid
        on Windows?
        \emph{Current status: unsupported. Both selected entry points are stale
        against current shared APIs; native build and smoke validation is
