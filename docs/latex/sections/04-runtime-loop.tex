...
int RunIllumoApplication(int argc, char** argv,
  ...
  illumo.addModule(application.createRequiredModule(), Required);
...
describes its CA options/help, supplies its defaults callback, and returns a
required \symbolref{CellGameModule}. It does not perform system work.
...
host initialization and passed to modules at \symbolref{Start}. It contains no
game validation helpers or product assumptions.
...
remaining rollback attempts. The engine runner registers the game-supplied
\symbolref{CellGameModule} as required and engine-owned Debug-build
\symbolref{DebugModule} as optional.
