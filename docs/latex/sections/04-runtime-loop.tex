...
\symbolref{RenderWindow} defaults to a GLFW swap interval of one. The persisted
\texttt{vsync} value can switch synchronized presentation off for uncapped
profiling and is observed at the frame boundary so Debug's generic environment
commands can change it live. The Debug overlay reports paced swap completions
separately from CPU frame submissions (D-P32).
