...
\begin{itemize}
  ...
  \item \textbf{Config} --- \symbolref{EnvVars} / \symbolref{IEnvVars}
        ...
        synchronized presentation via \texttt{vsync}, mode string,
        \texttt{WorldChunksX}/\texttt{WorldChunksY}, \ldots).
        The production file is \texttt{envvars.json}
        beside the executable, seeded from the tracked default only when absent;
        it never depends on the launch working directory. The Game-owned F1
        settings overlay updates the supported topology, timing, ruleset, and
        display variables in Release and Debug.
