...
Finite numeric and boolean inputs are validated. Unknown commands do not create
...
\begin{itemize}
  \item Save writes version 2 sparse metadata and deterministic chunk records,
        and reports stream failure.
  \item Load validates temporary sparse state before mutation, accepts both
        version 2 and the prior dense format, and rejects malformed payloads.
  \item A valid load activates its ruleset, replaces the sparse grid, restores
        saved camera metadata, rebuilds palette targets, and snaps the bounded
        display to the loaded state.
...
\begin{enumerate}
  ...
  \item The command queue now reserves 2,048, grows to a configurable 65,536
        default ceiling, and reports high-water/rejection metrics (D-R16).
\end{enumerate}
