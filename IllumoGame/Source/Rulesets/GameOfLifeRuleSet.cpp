...
unsigned char
GameOfLifeRuleSet::nextState(unsigned char cell,
                             unsigned char aliveNeighbors) const
{
  // B3/S23
  if (cell == CELL_ALIVE) {
    return (aliveNeighbors == 2 || aliveNeighbors == 3) ? CELL_ALIVE
                                                        : CELL_DEAD;
  }
...
void
GameOfLifeRuleSet::evalCell(const unsigned char& target,
