...
static void testFadeTowardTarget()
{
	...
	testTrue(g, f.canvas->isFadeActive() || f.canvas->getFadeSpeed() <= 0.0f, "fade active or instant");
	...
	testEqUChar(g, f.canvas->getDisplayTexBuffer()[base], 0, "fadeSpeed 0 snaps R");
}
...
static void testDirtyFlagsAndSparseUpload()
{
	...
	testTrue(g, !f.canvas->isTextureUploadPending(), "idle tick does not dirty texture");
	...
	testTrue(g, !f.canvas->isTextureUploadPending(), "paint alone does not upload display");

	f.canvas->rebuildTargetsFromLife();
	f.canvas->setFadeSpeed(0.0f);
	// Instant snap path in onTargetsRebuilt when fadeSpeed was already 0 after rebuild —
	// force snap after targets:
	f.canvas->setTargetColor(1 * 4 + 1, 0, 0, 0);
	f.canvas->snapVisualToTargets();
	testTrue(g, f.canvas->isTextureUploadPending(), "snap dirties display upload");
	const DirtyRect& ur = f.canvas->getUploadDirtyRegion();
	testEqInt(g, ur.minX, 1, "upload minX");
	testEqInt(g, ur.minY, 1, "upload minY");
}

static int runCanvasDomainCase(void (*testFunction)())
{
	g.failures = 0;
	testFunction();
	return g.failures;
...
void registerCanvasDomainTests(IllumoTestRegistry& registry)
{
	registry.add("Illumo.Canvas.ClearAndSetGet", []() { return runCanvasDomainCase(testClearAndSetGet); });
	registry.add("Illumo.Canvas.Dimensions", []() { return runCanvasDomainCase(testDimensions); });
	registry.add("Illumo.Canvas.ClearMarksUpload", []() { return runCanvasDomainCase(testClearMarksUpload); });
	registry.add("Illumo.Canvas.DefaultPalette", []() { return runCanvasDomainCase(testDefaultPalette); });
	registry.add("Illumo.Canvas.FadeTowardTarget", []() { return runCanvasDomainCase(testFadeTowardTarget); });
	registry.add("Illumo.Canvas.EnrollCreatesHandles", []() { return runCanvasDomainCase(testEnrollCreatesGpuHandles); });
	registry.add("Illumo.Canvas.DirtySparseUpload", []() { return runCanvasDomainCase(testDirtyFlagsAndSparseUpload); });
}
