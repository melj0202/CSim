...
static void testDayAndNightTruthTable()
{
	...
	{
		const bool shouldBirth = neighbors == 3 || neighbors == 6 || neighbors == 7 || neighbors == 8;
		const bool shouldSurvive = neighbors == 3 || neighbors == 4 || neighbors == 6 || neighbors == 7 || neighbors == 8;
		deadTransitionsMatch = deadTransitionsMatch
			&& rules.nextState(HeadlessCanvasFixture::Dead, neighbors)
				== (shouldBirth ? HeadlessCanvasFixture::Alive : HeadlessCanvasFixture::Dead);
		aliveTransitionsMatch = aliveTransitionsMatch
			&& rules.nextState(HeadlessCanvasFixture::Alive, neighbors)
				== (shouldSurvive ? HeadlessCanvasFixture::Alive : HeadlessCanvasFixture::Dead);
	}
...
static void testLifeWithoutDeathTruthTable()
{
	...
	for (unsigned char neighbors = 0; neighbors <= 8; ++neighbors)
	{
		deadTransitionsMatch = deadTransitionsMatch
			&& rules.nextState(HeadlessCanvasFixture::Dead, neighbors)
				== (neighbors == 3 ? HeadlessCanvasFixture::Alive : HeadlessCanvasFixture::Dead);
		aliveTransitionsMatch = aliveTransitionsMatch
			&& rules.nextState(HeadlessCanvasFixture::Alive, neighbors) == HeadlessCanvasFixture::Alive;
	}
	testTrue(g, deadTransitionsMatch, "dead cells are born only with three neighbors");
	...
	rules.evalCell(HeadlessCanvasFixture::Alive, rgb);
	testTrue(g, rgb[0] == 0 && rgb[1] == 0 && rgb[2] == 0, "alive cell is black");
	testTrue(g, rules.getRuleTag() == "LIFE_WITHOUT_DEATH", "Life Without Death rule tag");
}

static void testSeedsTruthTableAndColors()
{
	...
	for (unsigned char neighbors = 0; neighbors <= 8; ++neighbors)
	{
		const unsigned char expected = neighbors == 2
			? HeadlessCanvasFixture::Alive
			: HeadlessCanvasFixture::Dead;
		transitionsMatch = transitionsMatch
...
static void testBriansBrainStateMachineAndColors()
{
	testSection("Brian's Brain: state machine and colors");
	HeadlessCanvasFixture f(2, 2);
	BrainsBrainRuleSet rules(f.canvas);
	testEqUChar(g, rules.nextState(1, 2), 0, "dead with two neighbors becomes alive");
	testEqUChar(g, rules.nextState(1, 1), 1, "dead without two neighbors stays dead");
	...
	testTrue(g, rgb[0] == 0 && rgb[1] == 164 && rgb[2] == 128, "dying cell uses teal color");
	testTrue(g, rules.getRuleTag() == "BRIANS_BRAIN", "Brian's Brain rule tag");
}

static void testWireworldConductorNeighborCounts()
{
	testSection("Wireworld: conductor births with one or two heads only");
	HeadlessCanvasFixture f(2, 2);
	WireworldRuleSet rules(f.canvas);
	testEqUChar(g, rules.nextState(WireworldRuleSet::CELL_CONDUCTOR, 0), WireworldRuleSet::CELL_CONDUCTOR,
		"zero heads keeps conductor");
	testEqUChar(g, rules.nextState(WireworldRuleSet::CELL_CONDUCTOR, 1), WireworldRuleSet::CELL_HEAD,
		"one head creates electron head");
	testEqUChar(g, rules.nextState(WireworldRuleSet::CELL_CONDUCTOR, 2), WireworldRuleSet::CELL_HEAD,
		"two heads create electron head");
	testEqUChar(g, rules.nextState(WireworldRuleSet::CELL_CONDUCTOR, 3), WireworldRuleSet::CELL_CONDUCTOR,
		"three heads keep conductor");
}

static int runRuleSetCase(void (*testFunction)())
{
	g.failures = 0;
	testFunction();
	return g.failures;
...
void registerRuleSetTests(IllumoTestRegistry& registry)
{
	registry.add("Illumo.Rules.GameOfLifeBlock", []() { return runRuleSetCase(testGameOfLifeBlockStillLife); });
	registry.add("Illumo.Rules.GameOfLifeBlinker", []() { return runRuleSetCase(testGameOfLifeBlinker); });
	registry.add("Illumo.Rules.GameOfLifeEmpty", []() { return runRuleSetCase(testGameOfLifeEmptyStaysEmpty); });
	registry.add("Illumo.Rules.GameOfLifeColors", []() { return runRuleSetCase(testGameOfLifeEvalCellColors); });
	registry.add("Illumo.Rules.SeedsBirthOnly", []() { return runRuleSetCase(testSeedsBirthOnly); });
	registry.add("Illumo.Rules.BriansBrainTransition", []() { return runRuleSetCase(testBriansBrainAliveBecomesDying); });
	registry.add("Illumo.Rules.HighlifeTag", []() { return runRuleSetCase(testHighlifeRuleTag); });
	registry.add("Illumo.Rules.WireworldCycle", []() { return runRuleSetCase(testWireworldHeadTailConductorCycle); });
	registry.add("Illumo.Rules.WireworldElectron", []() { return runRuleSetCase(testWireworldElectronOnWire); });
	registry.add("Illumo.Rules.WireworldEmpty", []() { return runRuleSetCase(testWireworldEmptyStaysEmpty); });
	registry.add("Illumo.Rules.WireworldColors", []() { return runRuleSetCase(testWireworldEvalCellColors); });
	registry.add("Illumo.Rules.DayAndNightTruthTable", []() { return runRuleSetCase(testDayAndNightTruthTable); });
	registry.add("Illumo.Rules.HighlifeTruthTable", []() { return runRuleSetCase(testHighlifeTruthTable); });
	registry.add("Illumo.Rules.LifeWithoutDeathTruthTable", []() { return runRuleSetCase(testLifeWithoutDeathTruthTable); });
	registry.add("Illumo.Rules.SeedsTruthTable", []() { return runRuleSetCase(testSeedsTruthTableAndColors); });
	registry.add("Illumo.Rules.BriansBrainStateMachine", []() { return runRuleSetCase(testBriansBrainStateMachineAndColors); });
	registry.add("Illumo.Rules.WireworldConductorNeighbors", []() { return runRuleSetCase(testWireworldConductorNeighborCounts); });
}
