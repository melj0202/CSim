...
class CommandRegistry {
    ...
    public:
        ...
        bool QueueCommand(const std::string& name, const std::vector<std::string>& args = {}) {
            std::unordered_map<std::string, CommandFn>::iterator it = commands.find(name);
            if (it != commands.end()) {
        ...
        std::string GetCommandUsage(const std::string& name) const {
            std::unordered_map<std::string, std::string>::const_iterator it = usages.find(name);
            return it != usages.end() ? it->second : "";
        }
        std::string GetCommandDescription(const std::string& name) const {
            std::unordered_map<std::string, std::string>::const_iterator it = descriptions.find(name);
            return it != descriptions.end() ? it->second : "";
        }
        std::vector<std::string> GetCommandCompletions(const std::string& name) const {
            std::unordered_map<std::string, std::vector<std::string>>::const_iterator it = completions.find(name);
