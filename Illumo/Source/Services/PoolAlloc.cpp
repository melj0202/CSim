...
ChainedPoolAlloc<T>::ChainedPoolAlloc(size_t numBlocks)
{
  ...
  for (size_t i = 0; i < numBlocks; i++) {
    this->freeList.push_back(
      reinterpret_cast<T*>(this->chunk->data + (i * blockSize)));
  }
  this->numChunks++;
  chunkHead = chunk;
}
...
template<typename T>
void
ChainedPoolAlloc<T>::Deallocate(T* block)
{
  if (block == nullptr) {
    return;
  }
