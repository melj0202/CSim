...
static bool parseIntegerArgument(const std::string& text, int* value)
{
	...
	try
	{
		...
		if (consumed != text.size() || parsed < -2147483648LL || parsed > 2147483647LL)
		{
			return false;
		}
...
static bool parseFloatingArgument(const std::string& text, double* value)
{
	if (value == nullptr || text.empty())
	{
		return false;
...
static std::string withIllumoExtension(const std::string& filename)
{
	...
	if (filename.size() >= extension.size())
	{
		...
		{
			ending[i] = static_cast<char>(std::tolower(static_cast<unsigned char>(ending[i])));
		}
		if (ending == extension)
		{
			return filename;
...
void CellGameModule::Start(IllumoContext* context)
{
	...
	if (!IllumoContextHasGameCore(context))
	{
		Logger::LogError("CellGameModule::Start: IllumoContext missing required services "
			"(envVars, window, camera, renderer, inputManager, commandLine, commandRegistry, scene)");
		ic = context;
		return;
	}
	ic = context;
...
void CellGameModule::registerConsoleCommands()
{
	if (ic == nullptr || ic->commandRegistry == nullptr || ic->commandLine == nullptr)
	{
		return;
	}
	...
	CommandFn rulesetCommand = [this](const std::vector<std::string>& args)
	{
		if (args.empty())
		{
			ic->commandLine->logNormal("Current ruleset: " + cellContext->getModeString());
	...
	ic->commandRegistry->RegisterCommand(
		...
		{
			int generations = 1;
			if ((!args.empty() && !parseIntegerArgument(args[0], &generations)) || args.size() > 1 || generations < 1 || generations > 1000)
			{
				ic->commandLine->logError("step count must be an integer from 1 to 1000");
				return;
			}
			stepSimulation(generations);
			ic->commandLine->logSuccess("Advanced " + std::to_string(generations) + (generations == 1 ? " generation" : " generations"));
		},
		"step [count]",
	...
	ic->commandRegistry->RegisterCommand(
		...
		{
			double density = 25.0;
			if ((!args.empty() && !parseFloatingArgument(args[0], &density)) || args.size() > 1 || density < 0.0 || density > 100.0)
			{
				ic->commandLine->logError("randomize density must be a percentage from 0 to 100");
				return;
			}
			...
			for (int y = 0; y < canvas->canvasHeight; ++y)
			{
				for (int x = 0; x < canvas->canvasWidth; ++x)
				{
					...
					const unsigned char state = wireworld
						? (selected ? WireworldRuleSet::CELL_CONDUCTOR : WireworldRuleSet::CELL_EMPTY)
						: (selected ? static_cast<unsigned char>(0) : static_cast<unsigned char>(1));
					canvas->setCanvasPixel(x, y, state);
				}
			}
			ic->commandLine->logSuccess("Randomized canvas at " + std::to_string(density) + "% density");
		},
	...
	ic->commandRegistry->RegisterCommand(
		...
		[this](const std::vector<std::string>& args)
		{
			...
			int y = 0;
			int state = 0;
			if (args.size() != 3 || !parseIntegerArgument(args[0], &x) || !parseIntegerArgument(args[1], &y)
				|| !parseIntegerArgument(args[2], &state) || state < 0 || state > 255)
			{
				ic->commandLine->logError("Usage: setcell <x> <y> <state 0..255>");
				return;
			}
			if (!cellContext->getCellCanvas()->setCanvasPixel(x, y, static_cast<unsigned char>(state)))
			{
				ic->commandLine->logError("Cell coordinates are outside the canvas");
				return;
			}
			ic->commandLine->logSuccess("Cell (" + std::to_string(x) + ", " + std::to_string(y) + ") = " + std::to_string(state));
		},
		"setcell <x> <y> <state>",
	...
	ic->commandRegistry->RegisterCommand(
		...
		[this](const std::vector<std::string>& args)
		{
			if (args.empty())
			{
				...
				ic->commandLine->logNormal("Camera: x=" + std::to_string(position.x)
					+ " y=" + std::to_string(position.y) + " zoom=" + std::to_string(ic->camera->GetZoom()));
				return;
			}
			double x = 0.0;
			double y = 0.0;
			double zoom = static_cast<double>(ic->camera->GetZoom());
			if ((args.size() != 2 && args.size() != 3) || !parseFloatingArgument(args[0], &x)
				|| !parseFloatingArgument(args[1], &y)
				|| (args.size() == 3 && !parseFloatingArgument(args[2], &zoom)) || zoom < 0.1 || zoom > 100.0)
			{
				ic->commandLine->logError("Usage: camera <x> <y> [zoom 0.1..100]");
				return;
			}
			ic->camera->SetPosition(glm::vec2(static_cast<float>(x), static_cast<float>(y)));
...
void CellGameModule::unregisterConsoleCommands()
{
	...
	const char* commandNames[] = {
		"camera", "camera_reset", "clear_canvas", "load", "load_dialog", "mode", "pause", "randomize",
		"ruleset", "run", "save", "save_dialog", "setcell", "status", "step"
	};
...
void CellGameModule::setRunning(bool running)
{
	...
	ic->commandLine->logSuccess(running ? "Simulation running" : "Simulation paused in edit mode");
}

void CellGameModule::stepSimulation(int generations)
{
	currentState = CellState::EDIT;
	simAccum = 0.0;
	if (stateSplash != nullptr)
	...
	{
		cellContext->getRuleSet()->calcGeneration(0, 0, canvas->canvasWidth, canvas->canvasHeight);
	}
	updateVisualTargets();
}

void CellGameModule::printStatus() const
{
	const Canvas* canvas = cellContext->getCellCanvas();
	const glm::vec2 cameraPosition = ic->camera->GetPosition();
	ic->commandLine->logNormal(std::string("State: ") + (currentState == CellState::NORMAL ? "RUNNING" : "PAUSED/EDIT"));
	ic->commandLine->logNormal("Ruleset: " + cellContext->getModeString());
	ic->commandLine->logNormal("Canvas: " + std::to_string(canvas->canvasWidth) + " x " + std::to_string(canvas->canvasHeight));
	ic->commandLine->logNormal("Rate: " + ic->envVars->getVar("tps").value + " tps x "
		+ ic->envVars->getVar("speedFactor").value);
	ic->commandLine->logNormal("Camera: x=" + std::to_string(cameraPosition.x) + " y="
		+ std::to_string(cameraPosition.y) + " zoom=" + std::to_string(ic->camera->GetZoom()));
}
...
bool CellGameModule::SaveCellGame(std::string filename)
{
	if (filename.empty())
	{
	...
	const std::size_t tagBytes = std::min(activeTag.size(), static_cast<std::size_t>(MAX_RULETAG_SIZE - 1));
	...
	file.write(reinterpret_cast<const char*>(canvas->lifeCanvas), static_cast<std::streamsize>(cellCount));
...
bool CellGameModule::LoadCellGame(std::string filename)
{
	if (filename.empty())
	{
	...
	const long long fileCellCount = static_cast<long long>(fileWidth) * static_cast<long long>(fileHeight);
	const long long maxLoadCells = 100000000LL;
	if (fileWidth < 1 || fileHeight < 1 || fileCellCount < 1 || fileCellCount > maxLoadCells)
	{
		ic->commandLine->logError("Save contains invalid canvas dimensions");
		return false;
	}

	std::vector<unsigned char> loadedCells(static_cast<std::size_t>(fileCellCount));
	if (!file.read(reinterpret_cast<char*>(loadedCells.data()), static_cast<std::streamsize>(loadedCells.size())))
	{
		ic->commandLine->logError("Save is truncated before all cell data was read");
		return false;
	}
	...
	{
		const unsigned char* sourceRow = loadedCells.data() + static_cast<std::size_t>(y) * static_cast<std::size_t>(fileWidth);
		unsigned char* destinationRow = canvas->lifeCanvas + static_cast<std::size_t>(y) * static_cast<std::size_t>(canvas->canvasWidth);
	...
	{
		ic->commandLine->logWarning("Loaded overlap from " + std::to_string(fileWidth) + " x "
			+ std::to_string(fileHeight) + " save into " + std::to_string(canvas->canvasWidth) + " x "
