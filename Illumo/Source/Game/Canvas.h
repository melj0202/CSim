...
public:
	...
	__ILLUMO_FORCE_INLINE__ bool setCanvasPixel(const int& x, const int& y, const unsigned char& colorVal)
	{
		if (!inBounds(x, y))
		{
			return false;
	...
	__ILLUMO_FORCE_INLINE__ unsigned char getCanvasPixel(const int& x, const int& y) const
	{
		if (!inBounds(x, y))
		{
			return 1;
