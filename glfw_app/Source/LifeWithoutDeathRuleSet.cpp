...
void LifeWithoutDeathRuleSet::calcGeneration(const int x_start, const int y_start, const int x_end, const int y_end, CellCanvas& canvas) const
{
		...
		for (int j = 0; j < canvasVectorHeight; j += BLOCK_Y) {
			for (int by = 0; by < BLOCK_Y; by++) {
				for (int bx = 0; bx < BLOCK_X; bx++) {
					...
					if (temp == lifeStates.CELL_DEAD && ne[i + bx][j + by] == 3) {
						canvas.setCanvasPixel(i + bx, j + by, lifeStates.CELL_ALIVE);
					}
					else if (temp == lifeStates.CELL_ALIVE && ne[i + bx][j + by]  >= 0) {
						canvas.setCanvasPixel(i + bx, j + by, lifeStates.CELL_ALIVE);
					}
					else {
