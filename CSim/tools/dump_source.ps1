<#
.SYNOPSIS
  Concatenate first-party CSim source into a single plain-text file.

.DESCRIPTION
  Walks CSim project sources (Source/, Shader/, top-level CMakeLists.txt)
  and writes one file with clear per-file banners. Skips thirdparty/,
  build trees, binaries, and other non-source paths.

.PARAMETER OutFile
  Output path. Default: CSim/Source/all.txt (relative to repo CSim root).
...
.EXAMPLE
  .\CSim\tools\dump_source.ps1
  .\CSim\tools\dump_source.ps1 -OutFile .\CSim\csim_first_party.txt
#>
...
try {
    ...
    $sw.WriteLine("================================================================================")
    $sw.WriteLine("CSim first-party source dump (NOT thirdparty)")
