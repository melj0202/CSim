...
namespace Math {
    ...
    template<typename T>
    T vectorNormalize(const T& val) {
        return glm::normalize(val);
    }

    template<typename T>
    T valueNormalize(const T& a, const T& b) {
        return (b == 0) ? T(0) : a / b;
    }
    ...
    template<typename T>
    T quatLength(const T& q) {
        return glm::length(q);
    }


    template<typename T>
    T quatNormalize(const T& q) {
        return glm::normalize(q);
    }

    template<typename T>
    T quatDot(const T& a, const T& b) {
        return glm::dot(a, b);
    }
}