...
enum class MeshVertexLayout : int {
	// Canvas / triangle shaders: float pos3 | float color3 | float uv2 (stride 32)
	Pos3Color3Uv2 = 0,
...
public:
	IMesh() = default;
	IMesh(const std::vector<float>& vertexData, const std::vector<unsigned int>& indexData = {})
		: _vertexData(vertexData), _indexData(indexData) {}
	IMesh(const std::vector<float>& vertexData) : _vertexData(vertexData) {}
	virtual ~IMesh() = default;
	virtual void Destroy() = 0;

	unsigned int getVertexCount() const { return static_cast<unsigned int>(_vertexData.size() / 6); }
	unsigned int getIndexCount() const { return static_cast<unsigned int>(_indexData.size()); }
