...
void GameOfLifeRuleSet::evaluateNeighbors(unsigned char& cell, const unsigned char& ne, const int& x, const int& y) const
{
	if (cell == CellStates::CELL_ALIVE && (ne == 2 || ne == 3)) {
		setCanvasPixel(x, y, CellStates::CELL_ALIVE);
	}
	else if (!(cell == CellStates::CELL_ALIVE) && ne == 3) {
		setCanvasPixel(x, y, CellStates::CELL_ALIVE);
	}
	else {
